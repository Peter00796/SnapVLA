...
def unpack_tuple(fn: Callable[[Any], Tuple[Any]]) -> Callable[[Any], Any]:
    """Wrap a function that returns a tuple/list, extracting the first element.

    timm >= 1.0 changed get_intermediate_layers to return a list instead of a
    tuple, so we handle both cases here (transformers >= 4.50 compat patch).
    """
...
class PrismaticPreTrainedModel(PreTrainedModel):
    ...
    def _supports_sdpa(self) -> bool:
        ...
        Guard against attribute access during PreTrainedModel.__init__, which
        calls _check_and_adjust_attn_implementation before self.language_model
        is constructed (transformers >= 4.50 compatibility fix).
        """
...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    def __init__(self, config: PrismaticConfig) -> None:
        ...
        # Relaxed timm version check: original code required 0.9.x but timm 1.x is ABI-compatible
        # for the LayerScale / ViT features used here (transformers >= 4.50 compat patch).
        from packaging.version import Version as _V
        if _V(timm.__version__) < _V("0.9.10"):
            raise NotImplementedError(
                "TIMM Version must be >= 0.9.10; please raise a GitHub Issue "
                "if you urgently need support for earlier TIMM versions."
            )
